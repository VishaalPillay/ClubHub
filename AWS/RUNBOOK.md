...
>
> This runbook was written for an AWS deployment that was **designed, costed, and then
> deliberately not built** — see [`README.md`](./README.md) for why, and
> [`../docs/adr/0004-hosting-platform.md`](../docs/adr/0004-hosting-platform.md) for the decision.
> It is preserved verbatim as the design record, and because the reasoning in it (OIDC without
> static keys, secrets as discrete fields, migrations before traffic) carried over intact to the
> stack that actually shipped.
>
> **The live deployment runbook is [`../docs/DEPLOYMENT.md`](../docs/DEPLOYMENT.md).** Following
> the steps below will build real AWS infrastructure and bill you roughly **$46–52/month**.
>
> The CDK source these steps refer to lives at the annotated tag
> [`aws-cdk-final`](https://github.com/VishaalPillay/ClubHub/tree/aws-cdk-final/infra); it was
> removed from `main` in the migration.

This is the step-by-step guide to take ClubHub from your laptop to a live URL on AWS, and to
teach you what each step does. Architecture overview:
[diagrams/clubhub-aws-architecture.drawio](./diagrams/clubhub-aws-architecture.drawio), or the
Mermaid rendering in [`README.md`](./README.md).

**The shape of it:** the backend + all infrastructure are defined as code in the CDK app
(`infra/`, now at the `aws-cdk-final` tag). You run `cdk deploy` once from your laptop to bring
it up, then GitHub Actions redeploys on every push. The frontend runs on AWS Amplify Hosting,
connected straight to GitHub.
...
The [CI workflow](../.github/workflows/ci.yml) already runs tests on every push. To let the deploy
workflow run `cdk deploy` for you, give GitHub a role to assume — **without storing any AWS keys**.

> The deploy workflow itself (`.github/workflows/deploy.yml`) was **removed from `main`** during
> the migration — it triggered on every `backend/**` push and would have failed on an unset
> `AWS_DEPLOY_ROLE_ARN`, turning the Actions tab red. It is preserved at the `aws-cdk-final` tag,
> and its design is summarised in [ARCHITECTURE.md § CD](./ARCHITECTURE.md#cd--github-oidc-no-static-keys).
