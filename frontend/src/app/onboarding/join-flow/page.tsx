...
export default function JoinFlowPage() {
  ...
  const validateCode = async () => {
    ...
    setCodeError("");
    try {
      // We do a dry-run join just to resolve the code → club name + domains
      // Use a GET to lookup club by code (we'll add a public endpoint)
      const res = await api.get(`/clubs/lookup?code=${code.trim().toUpperCase()}`);
      setClubName(res.data.name);
      setClubId(res.data.id);
      setDomains(res.data.domains ?? []);
      setEnabledRoles(res.data.enabled_roles ?? null);
      goTo(2);
    } catch (e: unknown) {
