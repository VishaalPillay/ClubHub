...
import FeaturedProfilesCarousel from "@/features/dashboard/FeaturedProfilesCarousel";
...
import { listJoinRequests } from "@/lib/api/requests";
import { listMembers } from "@/lib/api/members";
import { listTasks } from "@/lib/api/tasks";
import { listEvents } from "@/lib/api/events";
import { isLeadPlus, isSecPlus as secPlus, isVPPlus as vpPlus } from "@/lib/roles";
import type { Announcement, AnnouncementScope, AnnouncementType } from "@/types/api";

export default function OverviewPage() {
  const { clubId, currentRole, domainId, userId } = useClub();
  ...
  // RBAC checks (display gating only — the API re-checks everything)
  const isSecPlus = secPlus(currentRole);
  const isVPPlus = vpPlus(currentRole);
  const canDraft = isLeadPlus(currentRole);
  const isMember = currentRole === 'member';
  const showMetrics = isSecPlus;

  const { data: announcements = [], isPending: loading } = useQuery({
    queryKey: ["club", clubId, "announcements"],
  ...
  // Metric tiles (exec view) — real counts from the list endpoints.
  ...
  const pendingJoinRequestsCount = joinRequests.length;

  const refetchAnnouncements = () =>
    queryClient.invalidateQueries({ queryKey: ["club", clubId, "announcements"] });
  ...
  const confirmDelete = async () => {
    if (deleteModalId !== null) {
      try {
        await deleteAnnouncement(clubId, deleteModalId);
        refetchAnnouncements();
      } catch {
  ...
  const confirmUpdate = async () => {
    if (updateData) {
      try {
        await updateAnnouncement(clubId, updateData.id, {
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
        ...
        <div className="lg:col-span-4 lg:border-l-1 border-black lg:pl-3 mt-4 lg:mt-0 pt-4 lg:pt-0 border-t-1 lg:border-t-0">
          ...
          {/* Editorial Image Block (Carousel — live leaderboard) */}
