...
interface WordmarkProps {
  ...
  className?: string;
  /**
   * Flip the palette for dark surfaces (the controls bar, the app's ink
   * footers): the scraps become pale and the letters go dark, so the mark keeps
   * its figure/ground instead of dissolving into the background.
   */
  invert?: boolean;
  title?: string;
}
 ...
 * angle.
 *
 * Mirrors frontend/src/components/ui/Wordmark.tsx, which draws the same geometry
 * against `.wired-*` classes. Regenerate both copies of wordmarkPaths.ts
 * together with scripts/gen-wordmark.mjs.
 *
 ...
 * most here: the nameplate is the largest thing on page one and squarely in the
 * LCP path.
 *
 * ── The colours are inline, and that is the fix for a real failure ───────────
 * They used to come from `--wm-ink` / `--wm-knock` in newspaper.css. When that
...
export default function Wordmark({
  ...
  const tile = invert ? "var(--np-paper, #f5f2ec)" : "var(--np-ink, #16150f)";
  /* Not pure black on the ink surfaces: the letterforms should read as printed
     on a pale scrap, not punched through it to the bar behind. */
